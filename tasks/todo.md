...
- [x] 修复体脂接口性别兼容缺陷
  - [x] `calculate_body_fat` 支持男/女与 male/female 等别名
  - [x] 增加围度与公式定义域参数校验，避免数学域错误

- [x] 修复聊天接口错误语义
  - [x] 当 Agent 内部失败时，`/api/chat/message` 返回 503 而非 200
  - [x] 保留 `HTTPException` 状态码，避免被统一异常覆盖为 500

- [x] 提升工具层参数鲁棒性
  - [x] 训练计划工具支持中英文 goal/level 入参
  - [x] 热量计算工具支持中英文 gender/activity/goal 入参
  - [x] 营养分析支持 `amount`/`weight` 双入参与常见食物英文别名

- [x] 回归验证
  - [x] 新增并通过边界测试（体脂英文性别、营养 weight 别名、chat 错误语义）
  - [x] `.venv` 下全量测试通过

### 2026-03-24
- Phase 18 已完成
  - 功能修复重点：体脂接口、聊天错误语义、工具参数标准化
  - 验证结果：`tests/run_tests.py` 全量通过

## Phase 17: PDF 实体映射版文档收敛 ✅

- [x] 读取工作区内 PDF 实体并提取可用结构
  - [x] 使用 `.venv` 安装并调用 pypdf 提取章节文本
  - [x] 明确 PDF 为结构参考，不复制游戏场景内容

- [x] 重写项目介绍文档（仅仓库实证）
  - [x] 将文档结构对齐为“定位-能力-难点-架构-验证-边界”
  - [x] 保留本项目真实实现映射与文件落点
  - [x] 输出为 md 文件并覆盖原文档

### 2026-03-24
- Phase 17 已完成
  - 已基于工作区 PDF 实体完成结构映射版项目文档重写
  - 文档路径：`docs/fitness_assistant_项目介绍.md`
  - 约束：内容仅来自仓库可验证事实，不使用推测信息